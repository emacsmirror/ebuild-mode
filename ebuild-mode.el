...
(define-minor-mode ebuild-repo-mode
  ...
  (unless (local-variable-p 'tab-width (current-buffer))
    (setq tab-width 4))
  (when (fboundp 'bug-reference-prog-mode)
    ;; separate whens to suppress a byte-compiler warning in XEmacs
    (when (and
	   ebuild-mode-enable-bug-reference
	   (not (local-variable-p 'bug-reference-bug-regexp (current-buffer)))
	   (not (local-variable-p 'bug-reference-url-format (current-buffer))))
